package com.nurseathome.bid.utils;
...
import static com.nurseathome.bid.utils.RequestContextUtils.getHeaderValue;
...
public class JwtUtils {
    ...
    public static String getAuthApplicationName() {
        return getPropertyFromToken("appName")
                .orElseThrow(() ->
                        new ResponseStatusException(BAD_REQUEST, "Нет информации об имени сервиса"));
    }

    public static String getCurrentUserEmail() {
        return getPropertyFromToken("email")
                .orElseThrow(() ->
                        new ResponseStatusException(BAD_REQUEST, "Нет информации об электронной почте"));
    }

    public static UUID getSsoUserIdFromToken() {
        ...
        return ofNullable(parseString(payload).getAsJsonObject().get("guid").getAsString())
                ...
                .orElseThrow(() -> new ResponseStatusException(BAD_REQUEST,
                        "Не удалось получить guid из JWT токена"));
    }

    public static String getCurrentRole() {
        return getPropertyFromToken("role")
                .orElseThrow(() ->
                        new ResponseStatusException(BAD_REQUEST, "Нет информации о роли пользователя"));
    }
